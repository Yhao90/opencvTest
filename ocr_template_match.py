...
# cv_show('yuan tu image', image)
image = myutils.resize(image, width=300)
gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
# cv_show('shu ru huidu tu', gray)
...
cv_show('在原图画出处理过后计算出的轮廓', cur_img)
...
for (i, (gX, gY, gW, gH)) in enumerate(locs):
    ...
    for c in digitCnts:
        ...
        # 在模板中计算每一个得分，digits是数组，0-9所有的数字
        for (digit, temp) in digits.items():
            # 模板匹配
            result = cv2.matchTemplate(roi, temp, cv2.TM_CCOEFF)
            (_, score, _, _) = cv2.minMaxLoc(result)
            scores.append(score)

        # 取出最大得分的数字放入groupOutput
